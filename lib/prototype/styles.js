...
prototype.prepareStyles = function() {
  ...
  var label = { normal: prefix({
    ...
    left: 0,
    height: height * 0.425,
    fontSize: props.fontSize || height * 0.3,
  ...
  var styles = Object.defineProperties({}, {
    ...
    label: { get: _=> {
      ...
      if (this.state.focused || this.value) return label.focused
      return label.normal
    }},
    ...
    messageInner:  { get: _=> {
      return this.state.message || this.props.saving
        ? messageInner.active
        : messageInner.normal
    }}
  })
  ...
  var classes = Object.defineProperties({}, {
    ...
    container: { get: _=> {
      ...
      if (this.state.focused || this.value) {
        names += ' floating ' + className
      }
      if (this.value) {
        names += ' title ' + className
      }
      if (this.props.disabled) {
        names += ' disabed ' + className
      }
      return names
    }},
    ...
    label: { get: _=> {
      let cls = 'label-' + HASH
      if (props.labelClass) cls += ' ' + props.labelClass
      return cls
    }}
  })
